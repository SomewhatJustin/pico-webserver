...
pub unsafe trait InterruptExt: InterruptNumber + Copy {
    ...
    fn is_enabled(self) -> bool {
        ...
        #[cfg(target_arch = "riscv32")]
        {
            xh3_is_enabled(self.number() as u16)
        }
    }
    ...
    fn is_pending(self) -> bool {
        ...
        #[cfg(target_arch = "riscv32")]
        {
            xh3_is_pending(self.number() as u16)
        }
    }
...
#[cfg(target_arch = "riscv32")]
fn xh3_interrupt_to_mask(irq_no: u16) -> (u16, u16) {
    let index = irq_no / 16;
    let bitmask = 1 << (irq_no % 16);
    (index, bitmask)
}

#[cfg(target_arch = "riscv32")]
fn xh3_interrupt_to_mask_index(irq_no: u16) -> u32 {
    let (index, bits) = xh3_interrupt_to_mask(irq_no);
    (bits as u32) << 16 | index as u32
}

#[cfg(target_arch = "riscv32")]
unsafe fn xh3_unmask(irq_no: u16) {
    let mask_index = xh3_interrupt_to_mask_index(irq_no);
    unsafe {
        core::arch::asm!("csrs 0xbe0, {0}", in(reg) mask_index);
    }
}

#[cfg(target_arch = "riscv32")]
fn xh3_mask(irq_no: u16) {
    let mask_index = xh3_interrupt_to_mask_index(irq_no);
    unsafe {
        core::arch::asm!("csrc 0xbe0, {0}", in(reg) mask_index);
    }
}

#[cfg(target_arch = "riscv32")]
fn xh3_is_enabled(irq_no: u16) -> bool {
    let (index, bits) = xh3_interrupt_to_mask(irq_no);
    let index = index as u32;
    let mut csr_rdata: u32;
    unsafe {
        core::arch::asm!("csrrs {0}, 0xbe0, {1}", out(reg) csr_rdata, in(reg) index);
    }
    (csr_rdata & ((bits as u32) << 16)) != 0
}

#[cfg(target_arch = "riscv32")]
fn xh3_is_pending(irq_no: u16) -> bool {
    let (index, bits) = xh3_interrupt_to_mask(irq_no);
    let index = index as u32;
    let mut csr_rdata: u32;
    unsafe {
        core::arch::asm!("csrrs {0}, 0xbe1, {1}", out(reg) csr_rdata, in(reg) index);
    }
    (csr_rdata & ((bits as u32) << 16)) != 0
}

#[cfg(target_arch = "riscv32")]
fn xh3_pend(irq_no: u16) {
    let mask_index = xh3_interrupt_to_mask_index(irq_no);
    unsafe {
        core::arch::asm!("csrs 0xbe2, {0}", in(reg) mask_index);
    }
}

#[cfg(target_arch = "riscv32")]
fn xh3_unpend(irq_no: u16) {
    let mask_index = xh3_interrupt_to_mask_index(irq_no);
    unsafe {
        core::arch::asm!("csrc 0xbe2, {0}", in(reg) mask_index);
    }
}
